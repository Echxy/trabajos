...
## Aqui lo que hice fue pegar las funciones para leer OFF Shapes 
...
# HICE OTRA VERSION QUE ACEPTE TEXTURAS
def createOFFShape_Tex(pipeline, filename,texturename, r,g, b):
    shape = readOFF(getAssetPath(filename), (r, g, b))
    gpuShape = es.GPUShape().initBuffers()
    pipeline.setupVAO(gpuShape)
    gpuShape.fillBuffers(shape.vertices, shape.indices, GL_STATIC_DRAW)
    gpuShape.texture = es.textureSimpleSetup(
    getAssetPath(texturename), GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR)
    return gpuShape
